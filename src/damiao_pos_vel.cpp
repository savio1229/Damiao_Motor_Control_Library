#include "damiao_pos_vel.hpp"

namespace damiao {

void Motor_Control_PosVel::init() {
    if (usb_ptr->Init() == -1) {
        while (1) { 
...
void Motor_Control_PosVel::usb_serial_write(const uint8_t* buf, uint16_t len) {
    if (!is_device_ready) return;
    acm_ptr->SndData(len, const_cast<uint8_t*>(buf));
}

void Motor_Control_PosVel::control_cmd(uint32_t id, uint8_t cmd) {
    usb_ptr->Task();
    if (!is_device_ready) return;
...
void Motor_Control_PosVel::enable(const Motor_PosVel& motor) {
    control_cmd(motor.GetSlaveId(), 0xFC);
}

void Motor_Control_PosVel::disable(const Motor_PosVel& motor) {
    control_cmd(motor.GetSlaveId(), 0xFD);
}

void Motor_Control_PosVel::control_pos_vel(const Motor_PosVel& motor, float output_pos_deg, float vel_rpm) {
    usb_ptr->Task();
    if (!is_device_ready) return;
